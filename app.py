...
def map_graph(df):
    map_fig = px.scatter_map(
        df, lat="LATITUDE", lon="LONGITUDE", color="PRICE", template="plotly_dark"
    )

    map_fig.update_layout(paper_bgcolor="#222")

    map_fig.update_layout(margin={"t": 0, "l": 0, "b": 0, "r": 0})
...
def bar_graph(df):
    bar_fig = px.histogram(
        df.groupby(["BUILD_YEAR"]).mean(numeric_only=True).reset_index(),
        x="BUILD_YEAR",
        y="PRICE",
...
def line_graph(df):
    line_fig = px.line(
        df.groupby(["BUILD_YEAR", "BEDROOMS"]).mean(numeric_only=True).reset_index(),
        x="BUILD_YEAR",
        y="PRICE",
        color="BEDROOMS",
        template="plotly_dark",
...
app.layout = html.Div(
    [
        ...
        dbc.Row(
            [
                dbc.Col([dcc.Graph(figure={}, id="map_graph")], width=6),
                dbc.Col([dcc.Graph(figure={}, id="bar_graph")], width=6),
            ]
        ),
        dbc.Row([dbc.Col([dcc.Graph(figure={}, id="line_graph")])]),
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H3(
                            "Predict the price of your house:",
...
def update_map_graph(suburb, bedroom, bathroom):
    ...
    filtered_df = df[
        df["SUBURB"].isin(filter_suburb)
        & df["BEDROOMS"].isin(filter_bedroom)
        & df["BATHROOMS"].isin(filter_bathroom)
    ]
